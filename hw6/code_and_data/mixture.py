...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        # Each row of i is the responsibilities for the ith example for each class
        sumR = np.sum(R, axis=0) # sum over all examples (e.g. sum columns)
        c = self.prior.a_mix # parameter of uniform dirichlet distribution being used to model prior over pi
        N, K = R.shape # number of training examples, number of classes
        pi = ((c-1) + sumR)/(K*(c-1) + N)
        return pi
